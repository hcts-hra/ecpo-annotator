...
<svg xmlns="http://www.w3.org/2000/svg" width="2000" height="6000">
    <g>
    {
        for $color at $index in $colors
        let $x := ($index * 200) mod 1600
        let $y := floor($index div 8) * 100
        return
        <g>
            <rect x="{ $x }" y="{ $y }" width="200" height="100" style="fill:{$color};" />
            <text x="{ $x + 10 }" y="{ $y + 20 }">{$color}</text>
        </g>
    }
    </g>
</svg>